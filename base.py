...
start = timedelta(hours=tuple_start[0],
minutes=tuple_start[1], seconds=tuple_start[2])
finish = timedelta(hours=tuple_finish[0],
minutes=tuple_finish[1], seconds=tuple_finish[2])
...
la_1_rad =
self.get_radians(self._list_longtitude[index][1:])
fo_2_rad = self.get_radians(self._list_latitude[index +
1])
la_2_rad = self.get_radians(self._list_longtitude[index+
1][1:])
list_of_distances.append(self.get_distance_in_meters(fo_1_rad, la_1_rad,
fo_2_rad, la_2_rad))
...
return radians
@staticmethod
def get_distance_in_meters(rad_fo_1, rad_la_1, rad_fo_2,
rad_la_2):
"""
...
delta_ce_de = atan(sqrt((cos(rad_fo_2)*sin(delta_la))**2 +
(cos(rad_fo_1)*sin(rad_fo_2) -
sin(rad_fo_1)*cos(rad_fo_2)*cos(delta_la))**2)/((sin(rad_fo_1)*sin(rad_fo_
2) + cos(rad_fo_1)*cos(rad_fo_2)*cos(delta_la))))
...
print(f"Сумарна довжина маршруту польоту:
{round(aircraft.get_sum_flight_distance(), 2)} метрів.")
print(f"Середня швидкість польоту:
{round(aircraft.get_middle_speed(), 2)} м/c.")
print(f"Мінімальна швидкість польоту:
{round(aircraft.get_min_max_speed_fly()['min'], 2)} м/c.")
print(f"Максимальна швидкість польоту:
{round(aircraft.get_min_max_speed_fly()['max'], 2)} м/c.")
print(f"Мінімальна висота польоту:
{round(aircraft.get_min_hight_value(), 2)} метрів.")
print(f"Максимальна висота польоту:
{round(aircraft.get_max_hight_value(), 2)} метрів.")
print(f"Загальний час усього польоту:
